...
import static edu.uci.ics.huymt2.service.api_gateway.GatewayService.*;
...
public class GatewayConfigs {
    ...
    public GatewayConfigs(ConfigsModel cm) throws NullPointerException {
        if (cm == null) {
            ...
            if (numThreads <= 0)
                ServiceLogger.LOGGER.info("No numThreads found. You are fucked!");
            else
                ServiceLogger.LOGGER.info("Found numThreads. Hahaha");

            requestDelay = Integer.parseInt(cm.getGatewayConfig().get("requestDelay"));
            if (requestDelay <= 0)
                ServiceLogger.LOGGER.info("No request delay found.");
            else
                ServiceLogger.LOGGER.info("Found requestDelay.");
        }
    }
